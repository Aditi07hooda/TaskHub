...
export default function TaskCreate({ projectid }) {
  ...
  useEffect(() => {
    if (project) {
      ...
      setTeamLeaderName(project.team_leader_name || "");
      const teamMembersArray = project.team_members
      ? project.team_members.split(',')
      : [];
      setTeamMembers(teamMembersArray);

      setSelectedDate(project.due_date ? new Date(project.due_date) : null);
      const parsedMemberTasks = JSON.parse(project.member_tasks || '[]');
      setMemberTasks(parsedMemberTasks);
    }
  }, [project]);
          ...
          <form action="#">
            <div className="grid gap-4 sm:grid-cols-2 sm:gap-6">
              ...
              <div className="relative max-w-sm">
                <div className="absolute inset-y-0 start-0 flex items-center ps-3.5 pointer-projects-none">
                  <svg
